...
def test_get_local_path():
    ...
    expected = os.path.join(SUB_DIR, "another.txt")
    # We pass FILE_IN_SUB_DIR as the 'local_filename' argument to get_local_path
    assert get_local_path("another.txt", FILE_IN_SUB_DIR) == expected
...
def test_get_parent_folder_level_zero_returns_filename():
    ...
    assert os.path.normpath(get_parent_folder(FILE_IN_SUB_DIR, level=0)) == os.path.normpath(os.path.basename(FILE_IN_SUB_DIR))
...
def test_get_parent_folder_level_too_high():
    ...
    root_of_relative_path = get_parent_folder(TEST_DIR, level=1) # This should be "" if TEST_DIR is "temp_test_path_dir"
