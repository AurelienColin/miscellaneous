...
class Kwargs:
    ...
    _cmap: Optional[LinearSegmentedColormap] = field(init=False, repr=False,
                                                     default=None)  # Defaulting to None, will be set by cmap property.
    ...
    def colorbar(self) -> Dict[str, Any]:  # colorbar arguments for plt.colorbar
        ...
        if self.colorbar_scale == 'log':
            if valid_vmin is not None and valid_vmax is not None and float(valid_vmin) > 0 and float(
                    valid_vmax) > 0:  # LogNorm needs positive values
    ...
    def logit_transform(self, x: Union[np.generic, np.ndarray]) -> Union[np.generic, np.ndarray]:
        ...
        epsilon_transformed = transform_without_scale(
            np.array(self.epsilon))  # Ensure epsilon is array for subtraction if x is array
...
class Display:
    ...
    def show(self: _DisplayType, display: bool = True, export_filename: Optional[str] = None,
    ...
    def format_ax(ax: matplotlib.axes.Axes, kwargs: Kwargs) -> None:
        ...
        effective_rotation = kwargs.xticks_rotation if kwargs.xticks_rotation is not None else (
            45.0 if kwargs.x_are_dates else 0.0)
    ...
    def plot_several_y(
        ...
        for i in indices_to_plot:
            ...
            series_specific_kwargs_dict.update(
                {'linestyle': ':', 'linewidth': 1, 'alpha': 0.5, 'labels': None, 'title': ''})
        ...
        # Percentiles
        q25, q50, q75 = np.nanpercentile(ys, [25, 50, 75],
                                         axis=1)  # Ensure percentile calculations handle potential all-NaN slices if any.

        self.plot(x, q25, ax=ax_to_use_for_all, kwargs=Kwargs(
            **{**base_plot_kwargs.__dict__, 'linestyle': '--', 'color': 'black', 'labels': ['25%'], 'title': ''}))
        self.plot(x, q50, ax=ax_to_use_for_all, kwargs=Kwargs(
            **{**base_plot_kwargs.__dict__, 'linestyle': '-', 'color': 'black', 'labels': ['50% (Median)'],
               'title': ''}))
        # The last plot call's kwargs will be used by decorator for overall formatting/show
        self.plot(x, q75, ax=ax_to_use_for_all, kwargs=Kwargs(
            **{**base_plot_kwargs.__dict__, 'linestyle': '--', 'color': 'black', 'labels': ['75%'],
               'title': base_plot_kwargs.title}))
    ...
    def plot_regression(
        ...
        if kwargs is None:
            kwargs = Kwargs()
        if ax is None:
            raise ValueError("Axes not provided by decorator for plot_regression")
        ...
        xmin = kwargs.xmin if kwargs.xmin is not None else float(np.nanmin(x_valid))
        xmax = kwargs.xmax if kwargs.xmax is not None else float(np.nanmax(x_valid))
        ymin = kwargs.ymin if kwargs.ymin is not None else float(np.nanmin(y_valid))
        ymax = kwargs.ymax if kwargs.ymax is not None else float(np.nanmax(y_valid))
        kwargs.xmin, kwargs.ymin, kwargs.xmax, kwargs.ymax = xmin, ymin, xmax, ymax

        dx = (xmax - xmin)/n
        dy = (ymax - ymin)/n

        x_indices = ((x - xmin)/dx).astype(int)
        y_indices = ((y - ymin)/dy).astype(int)
        ...
        z_density.add.at((x_indices, y_indices), valid)
        z_density[grid == 0] = np.nan

        im = ax.imshow(z_density, extent=(xmin, xmax, ymin, ymax), cmap=kwargs.cmap)

        rmse = float(np.sqrt(np.mean((y_valid - x_valid) ** 2)))
        bias = float(np.mean(y_valid - x_valid))
        pr: PearsonRResult = stats.pearsonr(x_valid, y_valid).statistic if len(x_valid) >= 2 else np.nan
        textstr = (f'RMSE: {rmse:.2g}\nBias: {bias:.2g}\nPearson Corr: {pr:.2%}')
        ax.text(0.05, 0.95, textstr, transform=ax.transAxes, va='top',
    ...
    def imshow(
        ...
        img: AxesImage = ax.imshow(array, cmap=kwargs.cmap, interpolation=kwargs.interpolation,
                                   aspect=kwargs.aspect, extent=kwargs.extent,
                                   **kwargs.colorbar)  # Important detail: Pass vmin, vmax, norm from colorbar property
    ...
    def heatmap(
        ...
        cbar_kws = {'orientation': kwargs.orientation, 'fraction': kwargs.fraction,
                    'format': fmt_val} if kwargs.orientation else None
        seaborn.heatmap(array[::-1], annot=True, ax=ax, fmt=fmt_val, cmap=kwargs.cmap,
                        vmin=kwargs.vmin if isinstance(kwargs.vmin, (int, float)) else None,
                        # Ensure vmin/vmax are numbers for heatmap
        ...
        if kwargs.labels and len(kwargs.labels) > 0:
            ...
            if array.shape[1] == num_labels: ax.set_xticks(ticks, kwargs.labels,
                                                           rotation=kwargs.xticks_rotation if kwargs.xticks_rotation is not None else 30.0,
                                                           ha='right')  # X-axis labels (columns)
            if array.shape[0] == num_labels: ax.set_yticks(ticks, kwargs.labels[::-1],
                                                           rotation=0)  # Y-axis labels (rows) - remember array is plotted [::-1]
    ...
    def undecorated_mesh(
        ...
        img: ScalarMappable = ax.pcolormesh(x_coords, y_coords, array, cmap=kwargs.cmap,
                                            shading='auto',
                                            **kwargs.colorbar)  # Explains shading='auto': Often needed if X, Y are not edge coordinates # Important detail: Pass vmin, vmax, norm
        if kwargs.aspect and hasattr(ax, 'set_aspect'): ax.set_aspect(
            kwargs.aspect)  # type: ignore # Explains hasattr: Basemap might not have set_aspect
        if kwargs.colorbar_display:  # Explains condition: Only add colorbar if display is true
            cbar_ax = ax if isinstance(ax, matplotlib.axes.Axes) else getattr(ax, 'ax',
                                                                              None)  # Important: Ensure ax for colorbar is matplotlib.axes.Axes, not Basemap directly if that causes issues # Explains getattr: Basemap.ax
            if cbar_ax: self.colorbar(img, ax=cbar_ax, orientation=kwargs.orientation, fraction=kwargs.fraction)
        if hasattr(ax, 'set_title'): ax.set_title(
            kwargs.title)  # type: ignore # Explains hasattr: Basemap might handle title differently or not at all
        return img
    ...
    @plot_decorator(
        format_ax=False)  # Explains decorator arg: format_ax=False because Basemap handles its own formatting
    def imshow_mercator(
            self: _DisplayType, array: np.ndarray, lat_grid: np.ndarray, lon_grid: np.ndarray,
            ax: Optional[matplotlib.axes.Axes] = None, kwargs: Optional[Kwargs] = None
            # Important: Basemap will be created on this Axes
    ) -> Basemap:
        if kwargs is None: kwargs = Kwargs()
        ...
        m = Basemap(projection=kwargs.projection, llcrnrlat=min_lat, urcrnrlat=max_lat,
                    llcrnrlon=min_lon, urcrnrlon=max_lon, resolution=kwargs.resolution,
                    ax=ax)  # Explains ax=ax: Draw Basemap on the provided matplotlib Axes

        lat_s, lon_s = float(kwargs.latitude_stride), float(kwargs.longitude_stride)
        if lat_s > 0: m.drawparallels(np.arange(np.floor(min_lat), np.ceil(max_lat), lat_s),
                                      labels=[1, 0, 0, 0])  # Explains labels: Left labels
        if lon_s > 0: m.drawmeridians(np.arange(np.floor(min_lon), np.ceil(max_lon), lon_s),
                                      labels=[0, 0, 0, 1])  # Explains labels: Bottom labels

        x_map, y_map = m(lon_grid, lat_grid)
        self.undecorated_mesh(array, x_map, y_map, ax=m,
                              kwargs=kwargs)  # Important detail: Pass Basemap instance as 'ax'
        m.fillcontinents(color=kwargs.earth_color, lake_color=kwargs.water_color)
        m.drawmapboundary(fill_color=kwargs.water_color)
        m.drawcountries();
        m.drawcoastlines()  # Explains choice: Optionally draw countries, coastlines
        # Useful: Title is handled by undecorated_mesh if ax=m supports set_title (Basemap does)
        return m

    @plot_decorator(format_ax=True)
    def barh(
            self: _DisplayType, y_categories: np.ndarray, x_values: np.ndarray,
            # Useful: Category names or positions for y-axis # Useful: Lengths of bars
            ax: Optional[matplotlib.axes.Axes] = None, kwargs: Optional[Kwargs] = None
        ...
        if kwargs.xscale == "logit":
            ...
            if kwargs.xmin is not None or kwargs.xmax is not None: logger.warning(
                "Logit barh xmin/xmax ignored.")  # type: ignore
            kwargs.xmax = kwargs.logit_transform(
                np.array(1.0 - kwargs.epsilon))  # Explains np.array: Use array for transform
            kwargs.xmin = kwargs.logit_transform(np.array(kwargs.epsilon))  # Explains np.array: Use array for transform
            x_plot_values = np.array([kwargs.logit_transform(v) if np.isfinite(v) and 0 < v < 1 else np.nan for v in
                                      x_values])  # Important condition: Only transform finite positive values for logit
            kwargs.xlabel = f"{kwargs.xlabel or ''} (logit scale)".strip()

        bc: BarContainer = ax.barh(y_categories, x_plot_values, color=kwargs.color,
                                   alpha=kwargs.alpha)  # Explains logic: Actual plot uses transformed values
    ...
    def plot_histogram(  # Explains purpose: For comparing multiple distributions
        ...
        for key, values in data_dict.items():
            finite_vals = [v for v in values if np.isfinite(
                v)]  # Important: Filter out NaNs before histogramming, as they cause issues or are ignored.
            if not finite_vals: continue  # Explains continue: Skip if no valid data
            ax.hist(finite_vals,
                    range=(kwargs.xmin, kwargs.xmax) if kwargs.xmin is not None and kwargs.xmax is not None else None,
                    # Explains logic: Ensure range is tuple or None
                    bins=kwargs.bins if kwargs.bins is not None else 'auto', alpha=kwargs.alpha,
                    # Explains choice: 'auto' is often a good default
                    density=kwargs.density, histtype=kwargs.histtype,
                    label=key + f" ({len(finite_vals)} pts)")  # Explains label: Add label for legend
        kwargs.ylabel = kwargs.ylabel if kwargs.ylabel else (
            'Density' if kwargs.density else 'Frequency')  # Explains choice: More standard terms
    ...
    def plot_single_histogram(  # Explains purpose: For detailed view of one distribution
        ...
        stat_line_y = (kwargs.ymax * 0.75) if kwargs.ymax is not None else (
                    current_ymax * 0.75)  # Explains choice: Heuristic: place stat lines at 75% of the (potentially new) max y if not set by user # Explains logic: If user set ymax, use that as reference

        ax.axvline(nanmean, color='r', linestyle='dashed', linewidth=kwargs.linewidth,
                   label=f'Mean: {nanmean:.3g}')  # Useful: Use from Kwargs
        ...
        kwargs.ylabel = kwargs.ylabel if kwargs.ylabel else (
            'Density' if kwargs.density else 'Frequency')  # Explains logic: Set default y-label based on density
    ...
    def rescale_axis(  # Explains: Instance method
        ...
        if x_label:
            ...
            ax.set_xticklabels([f'{x / current_factor:.2g}' for x in
                                ax.get_xticks()])  # type: ignore # Explains type ignore: get_xticks returns list of floats
            ax.set_xlabel(x_label)
        if y_label:
            ax.set_yticklabels([f'{y / current_factor:.2g}' for y in
                                ax.get_yticks()])  # type: ignore # Explains type ignore: get_yticks returns list of floats
